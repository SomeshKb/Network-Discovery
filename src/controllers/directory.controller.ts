...
export class DirectoryController {
    ...
    static async listFilesByExtension(req: Request, res: Response, next: NextFunction): Promise<void> {
        const networkPath = req.query.path as string;
    ...
    static async mountNetworkDirectory(req: Request, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            sendResponse(res, STATUS_CODES.OK, null, `${RESPONSE_MESSAGES.SUCCESSFULLY_MOUNTED} ${networkPath} to ${mountPoint}`);
        } catch (err) {
            next(err);
        }
    }

    static async unmountNetworkDirectory(req: Request, res: Response, next: NextFunction): Promise<void> {
        const { mountPoint } = req.body;

        try {
            await DirectoryService.unmountNetworkDirectory(mountPoint);
            sendResponse(res, STATUS_CODES.OK, null, `${RESPONSE_MESSAGES.SUCCESSFULLY_UNMOUNTED} ${mountPoint}`);
        } catch (err) {
            next(err);
        }
    }
