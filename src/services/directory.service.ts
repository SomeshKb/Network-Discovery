...
export class DirectoryService {
    ...
    static  listDirectories(networkPath: string): Promise<{ name: string; isDirectory: boolean }[]> {
        return new Promise((resolve, reject) => {
            fs.readdir(networkPath, { withFileTypes: true }, (err, files) => {
                if (err) {
                    return reject(new CustomError('Failed to list directories', STATUS_CODES.INTERNAL_SERVER_ERROR , err));
                }
    ...
    static listFilesByExtension(networkPath: string, extension: string): Promise<string[]> {
        return new Promise((resolve, reject) => {
            fs.readdir(networkPath, { withFileTypes: true }, (err, files) => {
                if (err) {
                    return reject(new CustomError('Failed to list files by extension', STATUS_CODES.INTERNAL_SERVER_ERROR, err));
                }
    ...
    static mountNetworkDirectory(networkPath: string, username: string, password: string, mountPoint: string): Promise<void> {
        return new Promise((resolve, reject) => {
            const mountArgs = [
            ...
            mountProcess.on('error', (err) => reject(new CustomError('Failed to mount directory', STATUS_CODES.INTERNAL_SERVER_ERROR, err)));
    ...
    static unmountNetworkDirectory(mountPoint: string): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            unmountProcess.on('error', (err) => reject(new CustomError('Failed to unmount directory', STATUS_CODES.INTERNAL_SERVER_ERROR, err)));
            unmountProcess.on('close', (code) => {
                if (code === 0) {
                    resolve();
                } else {
                    reject(new CustomError('Failed to unmount directory', STATUS_CODES.INTERNAL_SERVER_ERROR));
                }
            });
        });
    }
